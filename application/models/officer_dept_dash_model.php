...
class officer_dept_dash_model extends CI_Model
{
    ...
    public function getSnrMtoCounter()
    {
        ...
        $this->db->join('designation_history dh', 'e.id = dh.employee_id and dh.id = (select max(id) from designation_history where designation_history.employee_id = e.id)', 'inner');
...
from employee e
join designation_history dh on e.id = dh.employee_id and dh.id = (select max(id) from designation_history where designation_history.employee_id = e.id)
